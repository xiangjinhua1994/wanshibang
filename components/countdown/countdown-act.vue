<template>
	...
	<view class="countdown" :style="'color:'+color" :class="activeName">
		<template v-if="config.type == null">
			{{title}}<text class="daybox" :style="'background-color: '+backColor+';color:'+cutColor">{{ day }}天</text>
		...
		<template v-if="config.type == 'hours'">
			...
			<text class="box" :style="'background: '+backColor+';color:'+cutColor+';border-radius: 6rpx;padding: 4rpx;line-height: 1;font-size:24rpx;'">{{ parseInt(day * 24) + parseInt(hour) }}</text>
			<text class="p-0-10" :style="'color:'+color">:</text>
			<text class="box"  :style="'background: '+backColor+';color:'+cutColor+';border-radius: 6rpx;padding: 4rpx;line-height: 1;font-size:24rpx;'">{{ minute }}</text>
			<text class="p-0-10" :style="'color:'+color">:</text>
			<text class="box"  :style="'background: '+backColor+';color:'+cutColor+';border-radius: 6rpx;padding: 4rpx;line-height: 1;font-size:24rpx;'">{{ second }}</text>
			<text class="p-0-10" :style="'color:'+color"></text>
		</template>
		...
		<template v-if="config.type === 'preview'">
			{{title}}<text class="daybox" :style="'background-color: '+backColor+';color:'+cutColor">{{ day }}天</text>
			<text class="box" :style="'background-color: '+backColor+';color:'+cutColor">{{ hour }}</text>
			<text class="p-0-10" :style="'color:'+color">:</text>
			<text class="box" :style="'background-color: '+backColor+';color:'+cutColor">{{ minute }}</text>
			<text class="p-0-10" :style="'color:'+color">:</text>
			<text class="box" :style="'background-color: '+backColor+';color:'+cutColor">{{ second }}</text>
			<text class="p-0-10" :style="'color:'+color"></text>
		</template>
	</view>
</template>
		...
		methods: {
			...
			init() {
				...
				if (nowseconds < this.config.startstamp) {
					...
					this.countDown();
					this.title = this.startName;
				}else if (nowseconds > this.config.endstamp) {
					//活动已过期
					this.status = 2;
					this.title = this.endName;
				} else {
					//活动进行中
