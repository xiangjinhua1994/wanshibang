<template>
    ...
    <view class="head_top" :style="'height:'+topBarTop()+'px;'"></view>
        ...
        <view @click="searchFunc()" class="search-btn">搜索</view>
			</view>
			...
			<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'"
				...
				<view class="shop_body" v-if="isLieBiao ==true">
					<view class="shop_body_l_item" :class="index==listData.length-1?'noborder':''"
						...
						<view>
							<image :src="item.productStock === 0 ? item.soldImage : item.productImage" mode=""></image>
						</view>
				...
				<view class="shop_body2" v-if="isLieBiao ==false" txt="数据块状">
          <template v-for="(item, key) in listData" :key="key">
            <goodsBlock   :parentitem="item" @custom-event="receiveFromChild"></goodsBlock>
          </template>
				</view>
				<!-- 没有记录 -->
				<view class=" p30" v-if="listData.length==0 && !loading">
          <view class="search-not">
            <!-- <text class="iconfont icon-kongbaiye"></text> -->
            <image src="/static/home/null.png"  class="search-not-img"></image>
            <view class="cont">没有找到匹配的结果</view>
            <view class="search-proposal" v-if=" maybelick.length > 0" >您可以试试下面的建议搜索</view>
          </view>
          <view class="recommended" v-if=" maybelick.length > 0">
            <view class="recommended-title">推荐商品</view>
            <view class="recomm-main">
               
			<text  v-for="(item,index) in maybelick" :key="index" class="recomm-item-title">
              <view class="recomm-item">
				{{ item.simplifyTitle }}
			  </view>
			</text>	
             <!-- <view class="recomm-item">某某某商品</view>
              <view class="recomm-item">某某某商品</view>
              <view class="recomm-item">某某某商品</view>
              <view class="recomm-item">某某某商品</view>
              <view class="recomm-item">某某某商品</view>  -->
	  ...
	  spec
		...
		closePopup(e, cart_total_num) {
			...
			searchFunc() {
				...
				/*获取产品列表*/
				this.getData();
              if(this.listData.length==0){  //  搜索为空请求猜你猜你喜欢的产品
				let self = this;
				self._postBody('product/productRecommen/lists', {
				  code:"maybe"
				}, function(res) {
					if (res.code === 1) {
						self.maybelick = res.data.records;
					}
				});
			  }
			},
			...
			getData() {
				...
				for(let i = 0; i < self.listData.length; i++){
					productIdList.push(self.listData[i].productId)
				}
