<template>
	<view >
		...
		<view class="limited-spike m-0-20 d-b-s">
			...
			<view class="flex-1 pt16">
				<view class="white">
					...
					<text class="f42 fb">{{ subPrice(detail.productPrice, '1') }}.</text>
					<text class="f24 fb">{{ subPrice(detail.productPrice, '2') }}</text>
				</view>
				<view class="old-price">￥{{ detail.productSku.linePrice }}</view>
			</view>
			<view class="right-time">
				<Countdown
					activeName="previewProduct"
					ref="countdown"
					:config="{ startstamp: detail.preview.startTime, endstamp: detail.preview.endTime, type: 'preview' }"
					:start_name="'距开始仅剩'"
					:end_name="'距开始仅剩'"
		...
		<view class="bg-white  m-0-20 mb20 p20 mt-down-box">
			<view class="f30 gray3">{{ detail.productName }}</view>
			...
			<view class="border-t d-b-c mt20">
				...
				<button class="flex-1 active-btn d-c-c" @click="sendFunc('favorite')">
					<text :class="isFav ? 'icon-shoucang2 dominant' : 'icon-shoucang1'" class="icon iconfont shoucang-icon" src="/static/icon/sc.png" mode=""></text>
					<text class="f28" :class="isFav ? 'dominant' : 'gray9'">收藏</text>
				</button>
			</view>
		</view>
	</view>
