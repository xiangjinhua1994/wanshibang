	...
	<view>
		<view class="address-list bg-white">
			<view class="address p30 d-s-c border-b-e" v-for="(item,index) in storeList" :key="index">
				<view class="info flex-1" @click="onSelectedStore(item.storeId)">
					<view class="user f34">
					...
					<view>
						...
						<text>{{item.distanceUnit}}</text>
					</view>
					<!-- 选中状态 -->
					<view v-if="item.storeId == selectedId" class="shop-item__right">
...
<script>
	export default {
		...
		onLoad(options) {
			// 记录已选中的id
			this.selectedId = options.storeId;
			/*获取地址列表*/
			this.getLocation();
			this.getData();
		},
