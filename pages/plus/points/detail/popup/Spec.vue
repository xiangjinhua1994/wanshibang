<template>
	<view :data-theme='theme()' :class="Visible ? 'product-popup open '+ theme() : 'product-popup close '+ theme()"
		...
		<view class="main" v-on:click.stop>
			<view class="header">
				...
				<view class="price d-s-c">
					...
					<text class="num fb f34">{{ form.showSku.pointNum }}</text>
					<text>+</text>
					<text class="old-price">¥{{ form.showSku.pointMoney }}</text>
				</view>
				<!-- <view class="stock">库存：{{ form.showSku.pointStock }}</view> -->
				...
				<view class="level-box count_choose">
					...
					<view class="d-s-c">
						<view class="icon-box minus d-c-c" @click="sub()" :class="{ 'num-wrap': !issub }"><text
								class="icon iconfont icon-jian" style="font-size: 20rpx;color: #333333;"></text></view>
						...
						<view class="icon-box plus d-c-c" :class="{ 'num-wrap': !isadd }" @click="add()"><text
								class="icon iconfont icon-jia" style="font-size: 20rpx;color: #333333;"></text></view>
					</view>
				</view>
			</view>
		...
		methods: {
			...
			confirmFunc() {
				if (this.form.specData != null && !this.isAll) {
					console.log(this.form.specData);
			...
			/*初始化*/
			initShowSku() {
				this.form.showSku.skuImage = this.form.detail.product.image[0].filePath;
				this.form.showSku.pointPrice = this.form.detail.pointPrice;
				this.form.showSku.productSkuId = this.form.detail.sku[0].productSkuId;
				this.form.showSku.linePrice = this.form.detail.linePrice;
				this.form.showSku.pointStock = this.form.detail.stock;
				this.form.showSku.pointProductSkuId = this.form.detail.sku[0].pointProductSkuId;
				this.form.showSku.pointNum = this.form.detail.sku[0].pointNum;
				this.form.showSku.pointMoney = this.form.detail.sku[0].pointMoney;
			...
			updateSpecProduct() {
				...
				if (typeof skuItem === 'object') {
					let pointSkuList = self.form.detail.sku,
						pointSkuItem = pointSkuList.find(val => {
							return val.productSkuId == skuItem.productSkuId;
						});

					console.log(pointSkuItem);
					/*保存当前规格*/
					self.stock = pointSkuItem.pointStock;
					...
					self.form.showSku.pointPrice = pointSkuItem.pointPrice;
					self.form.showSku.pointProductSkuId = pointSkuItem.pointProductSkuId;
					self.form.showSku.pointStock = pointSkuItem.pointStock;

					if (skuItem.specForm.imageId > 0) {
						self.form.showSku.skuImage = skuItem.specForm.imagePath;
					} else {
						self.form.showSku.skuImage = self.form.detail.product.image[0].filePath;
					}
				}
			},
			...
			createdOrder() {
				let self = this;
				let pointProductId = self.form.detail.pointProductId;
				let num = self.form.showSku.sum;
				let productSkuId = self.form.showSku.productSkuId;
				let pointProductSkuId = self.form.showSku.pointProductSkuId;
