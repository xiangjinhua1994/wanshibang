	...
	<view class="prodcut-list-wrap" :data-theme="theme()" :class="theme() || ''">
		...
		<view class="list">
			<view class="item" v-for="(item, index) in listData" :key="index" @click="gotoList(item)">
				<view>
					<view class="f28">{{ item.recordName }}</view>
					<view class="f24 gray9">{{ item.createTime }}</view>
				</view>
				<button class="btn-red record-list-btn" @click="gotoPage('/pages/plus/lottery/receive?recordId=' + item.recordId)" v-if="item.status == 0">
					<span v-if="item.address == ''">未领取</span>
					<span v-if="item.address != ''">待发货</span>
				</button>
				<button v-else class="btn-gray record-list-btn">已领取</button>
			</view>
		</view>
		<!-- 没有记录 -->
