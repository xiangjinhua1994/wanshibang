		...
		<view class="group-bd bill-user-list">
			<view class="item d-b-c p20" v-for="(item, index) in bill" :key="index">
				<view class="userinfo d-s-c">
					<view class="photo"><image :src="item.avatarUrl" mode="widthFix"></image></view>
					<text class="ml10">{{ item.nickName }}</text>
				</view>
				<view class="btns d-s-c">
					<view class="d-s-c d-c">
						<text class="ml10">还差{{ item.difPeople }}人成团</text>
						<view class="gray9">
...
export default {
	...
	methods: {
		...
		/*转换参数*/
		rturnObjec(item){
			return {type:'text',startstamp:0,endstamp:item.endTime};
		},
