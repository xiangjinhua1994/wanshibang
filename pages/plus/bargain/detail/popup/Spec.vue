			...
			<view class="header">
				...
				<view class="price d-s-c">
					<template v-if="form.specData == null || isAll">
						<text>¥</text>
						<text class="num fb">{{ form.showSku.bargainPrice }}</text>
						<text class="old-price">¥{{ form.showSku.linePrice }}</text>
					</template>
					<template v-else>
						<text class="f22">¥</text>
						<text class="f40 fb">{{ form.detail.bargainPrice }}</text>
						<!--<text class="fb">-</text>
						<text class="f40 fb">{{ form.detail.bargain_high_price }}</text>-->
					</template>
				</view>
				<!-- <view class="stock">库存：{{ form.showSku.bargainStock }}</view> -->
					...
					<scroll-view scroll-y="true" class="specs mt20" style="max-height: 600rpx;" v-if="form.specData != null">
						<view class="specs mt20" v-for="(item_attr, attr_index) in form.specData.specAttr" :key="attr_index">
							<view class="specs-hd p-20-0">
								...
								<text class="ml10 red" v-if="form.productSpecArr[attr_index] == null">请选择{{ item_attr.groupName }}</text>
							</view>
...
export default {
	...
	methods: {
		...
		initShowSku() {
			...
			this.form.showSku.bargainPrice = this.form.detail.bargainPrice;
			this.form.showSku.productSkuId = 0;
			this.form.showSku.linePrice = this.form.detail.productPrice;
			this.form.showSku.bargainStock = this.form.detail.stock;
			this.form.showSku.bargainProductSkuId = this.form.detail.bargainSku[0].bargainProductSkuId;
		...
		updateSpecProduct() {
			...
			if (typeof skuItem === 'object') {
				let pointSkuList = self.form.detail.bargainSku,
					bargainSkuItem = pointSkuList.find(val => {
						return val.productSkuId == skuItem.productSkuId;
					});
				/*保存当前规格*/
				self.stock = bargainSkuItem.bargainStock;
				...
				self.form.showSku.bargainPrice = bargainSkuItem.bargainPrice;
				self.form.showSku.linePrice = skuItem.specForm.productPrice;
				self.form.showSku.bargainStock = bargainSkuItem.bargainStock;
				self.form.showSku.bargainProductSkuId = bargainSkuItem.bargainProductSkuId;
		...
		createdOrder() {
			let self = this;
			let params = {};
			params.bargainActivityId = self.form.detail.bargainActivityId;
			params.bargainProductId = self.form.detail.bargainProductId;
			params.bargainProductSkuId = self.form.showSku.bargainProductSkuId;
			params.productSkuId = self.form.showSku.productSkuId;
			params.productId = self.form.detail.productId;
			if (self.form.detail.specType != 20) {
				params.roductSkuId = self.form.detail.bargainSku[0].productSkuId;
				params.bargainProductSkuId = self.form.detail.bargainSku[0].bargainProductSkuId;
			}
			self._postBody('plus/bargain/task/add', params, function(res) {
				self.$emit('confirm');
				let bargainTaskId = res.data.bargainTaskId;
				let url = '/pages/plus/bargain/haggle/haggle?bargainTaskId=' + bargainTaskId + '&orderType=bargain';
				self.gotoPage(url);
			});
		},
