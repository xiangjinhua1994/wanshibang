		...
		<view v-for="(item,index) in tableData" :key='index'>
				...
				<view class="table-item">
					...
					<view class="uni-list">
						<radio-group @change="changeRadio($event,index)">
							<label class="d-s-c make-item mb20" v-for="(ritem,rindex) in getName(item.selectValue)"
								:key='rindex'>
								<view>
			...
			<template v-if="item.type == 'radio-group'">
				...
				<view class="table-item">
					...
					<view class="uni-list">
						<label  @click="changeRadioGroup(ritem,index)" class="d-s-c make-item mb20" v-for="(ritem,rindex) in getName(item.selectValue)"
							:key='ritem'>
							<view>
			...
			<template v-if="item.type == 'text'">
				...
				<view class="table-item">
					<view class="d-s-c make-top">
						<view class="fb color-28 f26"><text
								:class="item.isRequired!='false'?'red':'white'">*</text>{{item.name}}</view>
					</view>
					<view class="pr">
				...
				<view class="table-item">
					...
					<view class="uni-list">
						<picker class=""   @change="changeSelect($event,index)"
							:range="getName(item.selectValue)">
	...
	export default {
		...
		methods: {
			getData() {
				let self = this;
				self.loading = true;
				self._get('plus/table/table/toAdd', {
          ...
          this.tableData[index].value = arr.join(',');
			...
			changeSelect(e, index) {
				this.tableData[index].value = this.getName(this.tableData[index].selectValue)[e.detail.value];
			},
			/* 日期选择 */
			changeDate(e, index) {
				this.tableData[index].value = e.detail.value;
			...
			getImgsFunc(e) {
				if (e != null && e.length > 0) {
					this.tableData[this.uploadIndex].value = e[0].filePath;
				}
				this.uploadIndex = 0;
				this.isupload = false;
			},
			submit() {
				...
				self._postBody('plus/table/table/add', {
					content: params,
					tableId: self.tableId,
