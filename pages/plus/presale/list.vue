		...
		<view class="presale-list-wrap" v-if="!loading">
			<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'"
				...
				<view class="list p-0-20">
					<view class="item d-stretch" v-for="(item, index) in listData" :key="index" @click="gotoDetail(item.productId)">
						...
						<view class="d-b-c d-c flex-1 ml26">
							...
							<view class="d-s-c ww100">
								<Countdown ref="countdown" :color="'#999999'" :timeSize="'22rpx'"
									:config="{startstamp: new Date(item.startTime).valueOf() / 1000,endstamp: new Date(item.endTime).valueOf() / 1000}"
									@returnVal="returnValFunc"></Countdown>
							</view>
							<view class="ww100">
								<text class="dj-text">定金:￥{{ item.money }}</text>
								<text class="ml20  gray6 f20">尾款立减￥{{ item.sku[0].advancePrice }}</text>
							</view>
							<view class="slider-box ww100 d-b-c">
								<view>
									<view class="f32 redF11"><text
											class="f16 fb">￥</text>{{ (item.sku[0].productPrice*1 - item.sku[0].advancePrice*1).toFixed(2) }}
									</view>
	...
	export default {
		...
		data() {
			return {
				current: 0,
				banner: [{
					imgUrl: '/static/point_bg.png'
				}, {
					imgUrl: '/static/card.png'
				}],
				/*手机高度*/
				phoneHeight: 0,
				/*可滚动视图区域高度*/
