<template>
	<view class="login-container">
		...
		<view class="btns p30"
			><button type="default" @click="formSubmit">提交</button></view
		>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		formSubmit() {
			let self = this;
			/* if (!/^1(3|4|5|6|7|8|9)\d{9}$/.test(self.formData.value)) {
				uni.showToast({
					title: '邮箱有误,请重填！',
					duration: 2000,
					icon: 'none',
			...
			if (!self.testEmail(self.formData.value)) {
				uni.showToast({
					title: '邮箱有误,请重填！',
					duration: 2000,
					icon: 'none',
			...
			self.formData.type = 2; // 1:绑定手机 2:绑定邮箱
			self._postBody(
				'user/userOpen/bind',
				self.formData,
				(result) => {
