		...
		<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'" lower-threshold="50"
		 @scrolltolower="scrolltolowerFunc">
			<view class="my-bargaing-list">
				<view class="item" @click="gotoDetail(item.bargainTaskId)" v-for="(item, index) in listData" :key="index">
					<!--时间-->
					<view class="datetime d-b-c">
						<text class="f28">{{ item.createTime }}</text>
					...
					<view class="product d-s-s mt30">
						...
						<view class="info flex-1 ml20 f28">
							<text class="f26 text-ellipsis-2">{{ item.productName }}</text>
							<view class="mt20">
								<text class="redF6 f26">砍到底价：</text>
								<text class="redF6 f20">￥</text><text class="redF6 f32">{{ item.bargainPrice }}</text>
								<text class="ml10 gray9 text-l-t f26">{{item.productPrice}}</text>
							</view>
				...
				<view class="d-c-c p30" v-if="listData.length == 0 && !loading">
					...
					<text class="cont gray9">亲，暂无相关记录哦</text>
				</view>
				<uni-load-more v-else :color="'#333333'" :loadingType="loadingType"></uni-load-more>
			</view>
		</scroll-view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			/*查看详情*/
			gotoDetail(e) {
				let url = '/pages/plus/bargain/haggle/haggle?bargainTaskId=' + e;
				this.gotoPage(url);
			},
			goback() {
				uni.navigateBack();
