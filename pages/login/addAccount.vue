<template>
    <view class="web-login">
      <view class="login" txt="添加账号" v-if="pageType === 2">
        <view class="head_top" :style="'height:' + topBarTop() + 'px;'"></view>
        <headTitle title="添加账号">
        ...
        <view class="pas-login" v-if="registerType === 2" txt="邮箱">
          <view class="from-item">
            <view>
                <view class="uni-input">{{ '邮箱' }} </view>
              </view>
            <input type="text" class="search-input" placeholder-class="f24 gray9" placeholder="输入邮箱号"
          ...
          <view class="from-item">
            ...
            <text @click="sendCodeVery2">{{ send_btn_txt2 }}</text>
        ...
        <view class="pas-login" v-else-if="registerType === 1" txt="手机">
          <view class="from-item">
            <picker @change="regPickerChange" :value="index" :range="areaList" range-key="showName" mode=selector>
              <view class="uni-input" style="display: flex;">+ {{ areaList[index].tel }} <image style="width:10rpx;height:10rpx;position:relative;top:12rpx;left:3rpx" class="imgSize" src="/static/polygon.png" mode="" /> </view>
            </picker>
        ...
        <!-- <view @click="isRead = !isRead" class="xieyi-css">
          <image src="/static/home/unselected.png" mode="" v-if="isRead" style="margin-right:10rpx"/>
  ...
  export default {
    ...
    data() {
      return {
        ...
        index: 0, // 区号索引
        areaList: [],
        pageType: 2, // 0-找回密码 1-登录 2-注册 3-设置新密码
        smsOpen: false, // 是否开启短信验证
        loginType: 1, // 登录类型 1-手机验证码登录 2-邮箱验证码登录
    ...
    // 登录后， 获取用户信息， 渲染
      ...
      async smsLogin(e) {//  首页验证码登录
        ...
        if (self.registerType == 1) {
          ...
          if (!utils.isPoneAvailable(self.loginSmS.mobile)) {
            uni.showToast({
              title: '请输入正确的手机号',
              duration: 2000,
              icon: 'none',
          ...
          //  const res = await self.Verificationcode(parms)
           console.log(parms,'parms89898')
          //  debugger
          await self._post("user/userOpen/smslogin", parms, (res) => {
            console.log("登录返回",parms)
            //console.log(res)
            ...
            //   uni.setStorageSync('userId', res.data.loginUserVo.userId);
              ...
              if (newArr.indexOf(res.data.loginUserVo) === -1) {
                newArr.unshift(res.data.loginUserVo); // 如果新值不存在，添加新值
              }
      ...
      changeMsg2() { // 倒计时
        ...
        if (this.second2 > 0) {
            this.timerId = setTimeout(this.changeMsg2, 1000);
            this.send_btn_txt2 = this.second2 + 'S';
            this.second2--;

        } else {
            this.send_btn_txt2 = '获取验证码';
            this.second2 = 60;
            this.is_send = false;
        }
      },
