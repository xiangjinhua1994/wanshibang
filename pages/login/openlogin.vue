<template>
  <view :data-theme='theme()'
    ...
    <view class="skip"
          @click="gotoPage('/pages/index/index')">跳过→</view>
    <view class="p-30-75"
          v-if="is_login==2">
      <view class="login_topbpx">
        <view class="login_tit">注册</view>
        <view class="login_top">已有账户，<text class="dominant"
                @click="is_login=1">立即登录</text></view>
      </view>
      <view class="group-bd">
    ...
    <view class="p-30-75"
      ...
      <view class="login_topbpx">
        ...
        <view class="login_top">还没有账号，<text class="dominant"
                @click="is_login=2">立即注册</text></view>
      </view>
      <view class="group-bd">
        <view class="form-level d-s-c">
          <view class="val flex-1 input_botom"><input type="text"
    ...
    <view class="p-30-75"
      ...
      <view class="login_topbpx">
        ...
        <view class="login_top"><text class="dominant"
                @click="is_login=1">立即登录</text></view>
      </view>
      <view class="group-bd">
        <view class="form-level d-s-c">
    ...
    <view v-if="is_login==1"
      ...
      <view v-if="!is_code"
            @click="is_login=0">忘记密码?</view>
      <view @click="isCode()">{{is_code?'使用密码登录':'使用验证码登录'}}</view>
    </view>
    <view @click="isRead=!isRead"
          class="d-s-c gray6 p-0-75 mt20">
    ...
    <view style="padding-top: 80rpx;"
          class="btns p-30-75"
          v-if="is_login==2"><button @click="registerSub">注册</button></view>
    <view style="padding-top: 80rpx;"
          class="btns p-30-75"
          v-if="is_login==1"><button @click="formSubmit">登录</button></view>
    <view style="padding-top: 80rpx;"
          class="btns p-30-75"
          v-if="is_login==0"><button @click="resetpasswordSub">重置密码</button></view>
    <view class="bottom_nav"
          v-if="is_login==1">
      <view class="bottom-box">
        <view class="other_tit"><text class="bg-white p-0-20">其他方式登录</text></view>
        <view class="pt30 d-c-c"
              v-if="showWeixin">
          <view class="weixin_box"
                @tap="login"><text class="icon iconfont icon-weixin"></text></view>
        </view>
        <view v-if="showApple">
          <image @tap="appleLogin"
                 class="ios-login"
                 src="/static/ios.png"
...
export default {
  ...
  methods: {
    ...
    registerSub() {
      ...
      self.register.invitationId = uni.getStorageSync('invitationId')
            ...
            success(res) {
              ...
              self._postBody(
                'user/userapple/login',
                {
                  ...
                  nickName: res.userInfo.fullName.giveName
                    ? res.userInfo.fullName.giveName
                    : '',
